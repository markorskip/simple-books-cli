...
/**
 * A sequence event is something the user changes over time, and we need to use the latest change to it - therefore
 * we need a timestamp and not just date in order to resolve if the change happens twice in a day.  We need to record the timestamp so when all
